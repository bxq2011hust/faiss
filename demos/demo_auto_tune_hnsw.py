...
def plot_OperatingPoints(ops, nq, **kwargs):
    ops = ops.optimal_pts
    n = ops.size() * 2 - 1
    pyplot.plot([ops.at( i      / 2).perf for i in range(1,n)],
                [1/(ops.at((i + 1) / 2).t / nq) for i in range(1,n)],
                **kwargs)
...
keys_HNSW = [
    "HNSW40","HNSW40_SQ8","HNSW40_PQ32",
    "HNSW40_16384+PQ32","HNSW40_2x12+PQ32"
]
...
def run():
    ...
    for index_key in keys_to_test:
        ...
        if graphical_output:
            ...
            pyplot.ylabel("queries per second (1/s, %d threads)" % faiss.omp_get_max_threads())
