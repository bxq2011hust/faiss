...
//        index = faiss::index_factory(d, index_key);
        faiss::Clustering kmeans(d, k1);
        kmeans.niter = 25;
        faiss::IndexHNSWFlat *quant = new faiss::IndexHNSWFlat(d, M);
        quant->hnsw.efSearch = 128;
        kmeans.train(nt, xt, *quant);
        quant->reset();
        quant->add(kmeans.k, kmeans.centroids.data());
        index = new faiss::IndexIVFHNSW(quant, d, k1, M, w, factor);
        // faiss::IndexFlatL2 *coarse_quantizer = new faiss::IndexFlatL2(d);
        // index = new faiss::IndexIVFFlat(coarse_quantizer, d, 4096);
